...
【1. 趋势判定铁律】满足任意一条即可开多，否则禁止开仓：
   ...
   ✓ 条件C（新增-灵敏）：15m周期价格在MA{ma_slow}上方，且15m MA{ma_fast}向上（斜率>0）
   ✓ 条件D（新增-突破+量能）：价格突破1H压力位，且5m周期连续{chase_high_confirm_k}根K线站稳在突破位上方，**且突破时必须放量（5m量能相对值>{vol_break_threshold}）**，否则判定为假突破，禁止开仓
   
   量能约束（适用于条件C/D弱多头）：
   - 弱多头（条件C/D）开仓必须满足：量能状态为"放量"或"正常"，缩量上涨禁止开仓
   
   趋势强度分级（影响仓位）：
   - 强多头：1H周期MA{ma_fast}>MA10>MA{ma_slow}完全顺序排列 → 仓位50%~{max_position:.0f}%
   - 中等多头：满足条件A或条件B → 仓位40%~50%
   - 弱多头：仅满足条件C或D → 仓位{min_position:.0f}%~40%（试探性进场，严格止损，量能必须健康）

【2. 入场开仓条件】必须全部满足：
   ① 1H周期处于允许多头的状态（满足趋势判定）
   ② 入场位置：支撑位附近 或 突破压力位后回踩站稳{chase_high_confirm_k}~3根5分钟K线（压力转支撑）
   ③ 盈亏比 ≥ {min_risk_reward:.1f}
   ④ 止损空间 = 入场价 - 最近支撑位价格，且需在{min_sl_atr:.1f}~{max_sl_atr:.1f}倍1H ATR范围内
   ⑤ 禁止突破后立刻追高，只允许回踩企稳后开仓
   ⑥ 量能健康：突破开仓需放量（量能相对值>{vol_break_threshold}），回踩开仓需量能正常或缩量，量价背离（价格涨、量能缩）禁止开仓

【3. 止盈止损规则】
   - 唯一止损位：必须放在最近一级支撑位下方，触发立即全平
   - 双止盈设置：
     * TP1 = 入场价 + 1.1倍止损空间（平50%仓位）
     * TP2 = 入场价 + 2.0倍止损空间（平剩余仓位）
   - 止损空间计算：入场价 - 止损价
...
【5. 核心风控铁律】
   ① 单日总风险不超过本金{max_daily_risk_pct}%，超限当日停止开仓
...
{{{{
  ...
  "is_chase_high": false,
  "r": "满足条件A/B/C/D（需明确说明）+量能状态（放量/正常/缩量）+入场位于支撑位XX附近+盈亏比YY≥{min_risk_reward:.1f}+止损空间ZZ={min_sl_atr:.1f}~{max_sl_atr:.1f}倍ATR"
}}}}
说明：
- trend_strength: "strong"（1H完全多头）、"medium"（1H基本条件）或 "weak"（仅15m/突破确认）
- is_chase_high: true（条件D突破场景）或 false（常规开仓）
- e: 支持单一价格或区间格式（如"3.405-3.412"）
- r: 必须明确说明满足哪个条件（A/B/C/D），条件C需注明"15m周期"，条件D需注明"突破XX压力位"

2. 观望决策模板：
{{{{"d":"wait","r":"ma_not_allowed|risk_reward_low|direct_chase_high|atr_insufficient|cooling_down|no_signal|volume_insufficient|volume_abnormal"}}}}
原因说明：
- ma_not_allowed: 不满足趋势判定条件
- risk_reward_low: 盈亏比<{min_risk_reward:.1f}
- direct_chase_high: 突破后无回踩直接追高
- atr_insufficient: 止损空间不足{min_sl_atr:.1f}倍ATR
- cooling_down: 连续止损2次冷却中
- no_signal: 无明确入场信号
- volume_insufficient: 突破无量/量价背离（价格涨、量能缩），禁止开仓
- volume_abnormal: 回踩放量下跌，量能异常，禁止开仓

3. 平仓决策模板：
{{{{"d":"close","r":"stop_loss_hit|tp1_hit|tp2_hit|trend_reversal|cooling_down"}}}}
原因说明：
- stop_loss_hit: 触发止损
- tp1_hit: 触发第一止盈
- tp2_hit: 触发第二止盈
- trend_reversal: 趋势反转（不满足A/B/C/D任意一条）
- volume_divergence: 量价背离（价格涨量缩/价格跌量放）
