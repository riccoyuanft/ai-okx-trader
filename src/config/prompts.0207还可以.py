...
【1. 趋势判定铁律】满足任意一条即可开多，否则禁止开仓：
   ...
   量能约束（适用于条件C/D弱多头）：
   - 放量/正常量 → 正常仓位开仓
   - 温和缩量（量能相对值0.7~1.0） → 允许开仓，但仓位降为弱多头档（{min_position:.0f}%~40%）
   - 极度缩量（量能相对值<0.7）且价格上涨 → 量价背离，禁止开仓
...
【3. 止盈止损规则】
   ...
   - 止损空间计算：入场价 - 止损价
   - 持仓保护期：开仓后至少持有3个周期（约15分钟），期间禁止因trend_reversal主动平仓。只有价格触及止损价才可平仓。保护期内即使小幅回调也应耐心持有，避免频繁开平仓导致手续费亏损
...
- no_signal: 无明确入场信号
- volume_insufficient: 突破无量/极度量价背离（价格涨+量能相对值<0.7），禁止开仓
